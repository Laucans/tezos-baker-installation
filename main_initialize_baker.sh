...
echo "create tezos working folders"
...
echo "Stopping octez-node.service octez-baker.service octez-accuser.service if they exist and run ..."
sudo systemctl stop octez-node.service octez-baker.service octez-accuser.service

echo "Downloading tezos-binaries"
...
while ! sudo -u $baker_user /home/$baker_user/$octez_binaries_home/octez-client -d /home/$baker_user/$tezos_baker bootstrapped >/dev/null 2>&1; do
    ...
    ((count += 1))
    if ((count % 30 == 0)); then
        echo "Waiting for Tezos to bootstrap, $((count / 6)) minutes passed"
    fi
done
echo "Tezos is now fully bootstrapped"

echo "Initialize baker config"
./initialize/conf/initialize_baker_conf.sh $script_conf_filepath

echo "Initialize accuser config"
./initialize/conf/initialize_accuser_conf.sh $script_conf_filepath

#import ledger
echo "Link ledger keys"
./initialize/ledger/import_ledger.sh $script_conf_filepath

echo "Register and setup ledger to bake"
./initialize/ledger/register_and_setup_to_bake.sh $script_conf_filepath

echo "Enable and start octez-baker.service octez-accuser.service"
